...
public abstract class AbstractExecMojo extends AbstractMojo {
	...
	protected void collectProjectArtifactsAndClasspath(List artifacts,
		...
		if ("compile".equals(classpathScope)) {
			...
			theClasspathFiles.add(new File(project.getBuild()
					.getOutputDirectory()));
		...
		artifacts.addAll(this.pluginDependencies);
	 ...
	 * everything between STRING_WRAPPER. PARAMETER_DELIMITER is ignored inside
	 * a string. STRING_WRAPPER and PARAMETER_DELIMITER can be escaped using
	 * ESCAPE_CHAR.
	 * 
